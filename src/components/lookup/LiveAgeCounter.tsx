  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1 text-2xl sm:text-3xl md:text-4xl font-bold">
        <span><Slot value={frags.years.toString()} width={Math.max(2, frags.years.toString().length)} /> <span className="text-muted text-base font-medium">y</span></span>
        <span><Slot value={pad(frags.months)} width={2} /> <span className="text-muted text-base font-medium">mo</span></span>
        <span><Slot value={pad(frags.days)} width={2} /> <span className="text-muted text-base font-medium">d</span></span>
        <span><Slot value={pad(frags.hours)} width={2} /> <span className="text-muted text-base font-medium">h</span></span>
        <span><Slot value={pad(frags.minutes)} width={2} /> <span className="text-muted text-base font-medium">m</span></span>
        <span><Slot value={pad(frags.seconds)} width={2} /> <span className="text-muted text-base font-medium">s</span></span>
      </div>
